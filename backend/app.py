...
def get_video_status(video_id):
    db       = get_db()
    analysis = db.query(VideoAnalysis).filter_by(id=video_id, user_id=request.user_id).first()
    db.close()
    if not analysis:
...
def get_dashboard_stats():
    db       = get_db()
    analyses = db.query(VideoAnalysis).filter_by(user_id=request.user_id, status="done").all()
    db.close()
    ...
    total_workers_with_helmet = sum(max(0, a.estimated_people - a.inferred_violations) for a in analyses)
...
def generate_report(video_id):
    db       = get_db()
    analysis = db.query(VideoAnalysis).filter_by(id=video_id, user_id=request.user_id, status="done").first()
    db.close()
    if not analysis:
